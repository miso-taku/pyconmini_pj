...
| **TSP最適化** | python-tsp | >=0.4.0 | 巡回セールスマン問題の厳密解 |
...
#### python-tsp採用理由
1. **安定性**: mipライブラリの不安定性を回避
2. **パフォーマンス**: 動的計画法による高速な厳密解計算
3. **シンプル**: APIがシンプルで使いやすい
4. **依存関係**: 外部ソルバー不要（純Python実装）
...
**アルゴリズム**: 動的計画法（Held-Karp法）
- **ライブラリ**: `python-tsp`の`solve_tsp_dynamic_programming`
- **計算量**: O(n² × 2ⁿ)
- **実用範囲**: 15地点程度まで（数秒以内）
...
- [python-tsp Documentation](https://github.com/fillipe-gsm/python-tsp)
